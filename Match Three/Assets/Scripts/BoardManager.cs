...
public class BoardManager : MonoBehaviour
{
    ...
    private void CreateBoard(Vector2 tileSize)
    {
        ...
        for (int x = 0; x < size.x; x++)
        {
            ...
            {
                TileController newTile = Instantiate(tilePrefab, new Vector2(startPosition.x + ((tileSize.x + offsetTile.x) * x),
                    startPosition.y + ((tileSize.y + offsetTile.y) * y)), tilePrefab.transform.rotation, transform).GetComponent<TileController>();

                tiles[x, y] = newTile;

                // get no tile id
                List<int> possibledId = GetStartingPossibleIdList(x, y);
                int newId = possibledId[Random.Range(0, possibledId.Count)];
    ...
    public List<TileController> GetAllMatches()
    {
        ...
        for (int x = 0; x < size.x; x++)
        {
            for (int y = 0; y < size.y; y++)
            {
                List<TileController> tileMathed = tiles[x, y].GetAllMatches();

                // just go to next tile if no match
                if (tileMathed == null || tileMathed.Count == 0)
                {
                    continue;
                }

                foreach (TileController item in tileMathed)
                {
            ...
            for(int y = 0; y < size.y; y++)
            {
                if (tiles[x, y].IsDestroyed)
                {
                    ...
                    for(int i = y + 1; i < size.y; i++)
                    {
                        ...
                        // if this already on drop list, increase its drop range
    ...
    private IEnumerator DestroyAndFillTiles(List<TileController> destroyedTiles, System.Action onCompleted)
    {
        ...
        foreach(TileController tile in destroyedTiles)
        {
            ...
            Debug.Log("target " + targetIndex.x + " " + targetIndex.y);
